...
void *AtenderCliente (void *socket)
{
	...
	while (terminar ==0)
	{
		...
		if (codigo ==0) //petici?n de desconexi?n
		...
		else if (codigo == 4)//quiere saber si es palíndromo
			{
                int polindromo = 0;
				int longitud = strlen(nombre);
				if(longitud >1){
					for (i = 0, j = longitud - 1; i < longitud / 2; i++, j--) {
                        if (nombre[i] != nombre[j] && nombre[i] != tolower(nombre[j])) {
                            polindromo = 1;
						}
					}
				}
		...
		else //quiere devolver su nombre en mayúsculas
			{
			...
			// Devolvemos el nombre a mayúsculas.
